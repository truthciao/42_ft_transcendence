...
export function SpaceMembersPage() {
  ...
  async function handleRemove(memberUserId: number, username: string) {
    const confirmed = await confirm({
      title: t('workspaces.pages.members.removeConfirmTitle', {
        name: username,
      }),
    ...
    try {
      ...
      toast.success(
        t('workspaces.pages.members.removeSuccess', { name: username }),
      );
    } catch {
      ...
      <header className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">
            {t('workspaces.pages.members.title')}
          </h1>
          <p className="text-muted-foreground">
            {t('workspaces.pages.members.subtitle')}
          </p>
        </div>
        <PermissionButton
              ...
              >
                  ...
                  <p className="truncate text-sm font-medium">
                    ...
                    {isSelf ? (
                      <span className="ml-1.5 text-xs text-muted-foreground">
                        {t('workspaces.pages.members.you')}
                      </span>
                    ) : null}
                  </p>
                ...
                {isOwner ? (
                  <span className="rounded-full bg-muted px-2.5 py-1 text-xs font-medium">
                    {t('workspaces.pages.members.owner')}
                  </span>
            ...
            <div>
              <p className=" font-medium">
                {invite.invitee?.profile?.displayName ||
                  ...
                  t('workspaces.pages.members.pendingInvites.unknown')}
              </p>
